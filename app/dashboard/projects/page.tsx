...
import {
  ...
  Share2, Download, Archive, Trash2, FolderOpen, ArrowUpRight, Filter,
} from "lucide-react"
import {
...
export default function ProjectsPage() {
  ...
  const filtered = activeProjects.filter((p) => {
    ...
    const matchCategory = categoryFilter === "全部" || (p.tags || []).includes(categoryFilter)
  ...
  const handleArchive = (id: string) => {
    ...
    if (proj) updateProject(id, { isArchived: !proj.isArchived })
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
            ...
            <Link key={project.id} href={`/dashboard/projects/${project.id}`}
              ...
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground truncate">{project.description || "暂无简介"}</p>
                  <div className="flex items-center gap-2 mt-1">
                    {(project.tags || []).map((tag) => (
                      <span key={tag} className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{tag}</span>
                    ))}
                    <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{project.storyLength}</span>
                    <span className="text-xs text-muted-foreground">未完成</span>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-xs text-muted-foreground hidden sm:block">{new Date(project.updatedAt).toLocaleDateString("zh-CN")}</span>
                <Button variant="ghost" size="sm" className="opacity-0 group-hover:opacity-100 transition-opacity"><ArrowUpRight className="h-4 w-4" /></Button>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" onClick={(e: React.MouseEvent) => e.preventDefault()}>
                    ...
                    <DropdownMenuItem onClick={() => handleArchive(project.id)}>
                      <Archive className="h-3.5 w-3.5 mr-2" />{project.isArchived ? "取消归档" : "归档"}
                    </DropdownMenuItem>
                    <DropdownMenuItem className="text-red-500" onClick={() => setDeleteDialog({ open: true, id: project.id, name: project.name })}>
      ...
      {archivedProjects.length > 0 && (
        <div>
          <h3 className="text-sm font-medium text-muted-foreground mb-2">已归档 ({archivedProjects.length})</h3>
          <div className="space-y-1">
            {archivedProjects.map((project) => (
              <div key={project.id} className="flex items-center justify-between p-3 rounded-lg border border-border/50 opacity-60 hover:opacity-100 transition-opacity">
                <p className="text-sm">{project.name}</p>
                <Button variant="ghost" size="sm" onClick={() => handleArchive(project.id)}>取消归档</Button>
              </div>
            ))}
          </div>
        </div>
