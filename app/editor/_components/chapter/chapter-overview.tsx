      ...
      <div className="space-y-3">
        {subSections.length === 0 ? (
          ...
          subSections.map((ss, idx) => (
            <button key={ss.id} onClick={() => onSubSectionClick(ss.id)}
              ...
              <div className="flex items-center gap-3 p-4">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-semibold bg-gradient-to-br from-pink-100 to-violet-100 text-pink-600">{idx + 1}</div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-medium text-foreground truncate">{ss.title}</h4>
                  <span className="flex items-center gap-1 text-xs text-muted-foreground">
                    <MessageSquare className="h-3 w-3" />{ss.dialogues.length} 条对话
                  </span>
                </div>
              </div>
            </button>
...
// 对话数据自动通过 Zustand store → IndexedDB(即时) → 30s 后 PATCH /full 同步到服务器
export function SubSectionEditor({
  subSection, onBack, onUpdate, allSubSections, chapterId: _chapterId,
}: {
  ...
  const flatIds = allSubSections.flatMap(ch =>
    (ch.children || []).map(c => ({ id: c.value, title: c.label }))
  )
  ...
  return (
    <div>
      ...
      <SubSectionCard subSection={subSection} index={0} isExpanded={true}
        onToggle={() => {}} onUpdate={onUpdate}
        allSubSections={flatIds} subSectionTree={allSubSections} />
    </div>
  )
}
