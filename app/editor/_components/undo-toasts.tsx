...
export function UndoToasts() {
  ...
  return (
    <>
        ...
        <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 animate-slide-up">
          <div className="flex items-center gap-3 rounded-lg bg-gray-900 px-4 py-3 text-white shadow-lg">
            <span className="text-sm">
              已删除「{deletedChapter.chapter.title}」
            </span>
            <button
              onClick={undoDelete}
      ...
      {/* Ending Undo Toast */}
      {showEndingUndoToast && deletedEnding && (
        <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 animate-slide-up">
          <div className="flex items-center gap-3 rounded-lg bg-gray-900 px-4 py-3 text-white shadow-lg transition-opacity duration-500" style={{ opacity: showEndingUndoToast ? 1 : 0 }}>
            <span className="text-sm">
              已删除「{deletedEnding.ending.name}」
            </span>
            <button
              onClick={undoDeleteEnding}
              className="rounded bg-white/20 px-3 py-1 text-sm font-medium hover:bg-white/30"
            >
              撤销
            </button>
